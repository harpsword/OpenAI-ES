...
def get_reward(modeltype, base_model, envi, ep_max_step, sigma, CONFIG, seed_and_id=None, test=False):
    # start = time.time()
...
def train(model, optimizer, pool, sigma, env, N_KID, CONFIG, modeltype):
    ...
    jobs = [pool.apply_async(get_reward, (modeltype ,model, None, CONFIG['ep_max_step'],sigma,CONFIG,
...
def test(model, pool, env, test_times, CONFIG):
    # distribute training in parallel
    jobs = [pool.apply_async(get_reward, (None,model, None, CONFIG['ep_max_step'], None, CONFIG, None, True)) for i in range(test_times)]
    from config import timesteps_per_batch
    # N_KID means episodes_per_batch
    rewards = []
    timesteps = []
