...
def main(namemark, ncpu, batchsize, generation, lr, sigma, vbn, vbn_test_g, gamename, logfile,modeltype):
    ...
    for g in range(generation):
        ...
        model, kid_rewards, timestep_count = train(model, optimizer, pool, sigma, env, int(batchsize/2), CONFIG, modeltype, reference_batch_torch)
        ...
        if g % 5 == 0:
            test_rewards, timestep_count = test(model, pool, env, test_times, CONFIG, reference_batch_torch)
            test_rewards_mean = np.mean(np.array(test_rewards))
            experiment_record['test_rewards'].append([g, test_rewards])
